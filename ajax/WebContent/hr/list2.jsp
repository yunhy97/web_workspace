...
<script type="text/javascript">
	function openPopup(event) {
		...
		xhr.onreadystatechange = function() {
			//콜백함수가 4번 실행되기는 하지만, 실질적인 작업이 구현된 아리의 소스는
			...
			if(xhr.readyState == 4 && xhr.status == 200) {
				...
				document.getElementById('phone-cell').textContent = emp.phoneNumber;
	...
	function getEmployees() {
		...
		//*서버가 보낸 응답데이터가 JSON형식의 데이터라면 JSON.parse() 함수를 사용해서
		// javascript의 배열이나 객체로 변환할 수 있다.
		//*배열이나 객체로 변환되었다면 자바스크립트 문법을 사용해서 값을 조회하고,
		// 조회된 값을 이용해서 HTML컨텐츠를 만든 다음, 특정 태그에 부착시켜서 화면에
		// 표시되게 한다.
		xhr.onreadystatechange = function() {
			if(xhr.readyState == 4 && xhr.status == 200) {
				var text = xhr.responseText;
				var employees = JSON.parse(text);	//javascript의 배열로 변환되어 있다.
