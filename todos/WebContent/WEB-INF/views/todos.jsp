						...
						<div class="col-6 ">
							<ul class="pagination">
								<c:if test="${not pagination.first }">
									<li class="page-item"><a class="page-link" href="#" onclick="refreshList(${pagination.page - 1},event)">이전</a></li>
								</c:if>
								<c:forEach var="num" begin="${pagination.begin }" end="${pagination.end }">
									<li class="page-item ${pagination.page eq num ? 'active' : '' }"><a class="page-link" href="#" onclick="refreshList(${num},event)">${num }</a></li>
								</c:forEach>
								
								<c:if test="${not pagination.last }">
									<li class="page-item"><a class="page-link" href="#" onclick="refreshList(${pagination.page + 1},event)">다음</a></li>
								</c:if>
							</ul>
						</div>
...
<script type="text/javascript">
	function refreshList(pageNo, event){
		...
		//<form>안의 <input tupe="hidden" name="pageNo" /> 의 값을 설정한다.
